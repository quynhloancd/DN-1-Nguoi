...
export async function POST(req: NextRequest) {
  try {
    ...
    if (order.user_id && order.product_id) {
      try {
        console.log(`[PayOS Webhook] Step: upsert_enrollment`, {
          orderId: order.id,
        ...
        if (enrollErr) {
          console.error("[PayOS Webhook] CRITICAL: enrollment failed after payment", {
            orderId: order.id,
            userId: order.user_id,
            productId: order.product_id,
            error: enrollErr,
          });
          // Không throw — vẫn tiếp tục để PayOS không retry webhook
        } else {
          console.log(`[PayOS Webhook] Step: upsert_enrollment SUCCESS`, { orderId: order.id });
        }
      } catch (enrollErr) {
        console.error("[PayOS Webhook] CRITICAL: enrollment failed after payment", {
          orderId: order.id,
        ...
        // Không throw — vẫn return 200 để PayOS không retry
      }
