...
const CreatePostSchema = z.object({
  content: z.string().min(1, "Nội dung không được để trống").max(5000, "Nội dung quá dài (tối đa 5000 ký tự)"),
  tags: z.array(z.string().max(50)).max(20).optional(),
  image_url: z.string().url("URL ảnh không hợp lệ").optional().or(z.literal("")),
  category: z.string().max(100).optional(),
  product_id: z.string().uuid("product_id không hợp lệ").optional(),
});
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { content, tags, image_url, category, product_id } = parseResult.data;

    let sanitizedImageUrl: string | null = image_url ?? null;
    if (image_url) {
      try {
        ...
        if (!["http:", "https:"].includes(urlObj.protocol)) {
          ...
          const isAllowedImageUrl = (url: string) => {
            ...
            try {
              ...
              return parsed.hostname === supabaseHostname || parsed.hostname.endsWith(".supabase.co");
            } catch { return false; }
          };
