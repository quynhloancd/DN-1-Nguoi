...
export async function GET(req: NextRequest) {
  // Require secret token for detailed health info — public callers get only status
  ...
  // Special check: stuck orders — requires authentication
  if (check === "orders") {
    if (!isAuthenticated) {
  ...
  // Unauthenticated callers only get the top-level status — no internal details
  if (!isAuthenticated) {
