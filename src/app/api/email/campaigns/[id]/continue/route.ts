...
type SESv2Client = unknown;

function createSESClient(): SESv2Client {
  // No-op: emails routed through Resend driver.
  return {} as SESv2Client;
}
  ...
  try {
    ...
    for (const send of queuedSends) {
      ...
      try {
        ...
        // Send via Resend
        void sesClient;
        const sendResult = await sendEmailWithParams({
          to: send.email,
          subject: campaign.subject,
          html: renderedHtml,
          text: campaign.text_content || undefined,
          fromName: campaign.from_name || process.env.EMAIL_FROM_NAME || "KDON Academy",
        ...
        const result = { MessageId: sendResult.messageId };
