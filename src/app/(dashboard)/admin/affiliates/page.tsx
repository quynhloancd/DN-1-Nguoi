...
export default async function AdminAffiliatesPage({
  ...
  // Pending conversions
  const { data: pendingConversionRows } = await admin
    .from("affiliate_conversions")
    .select("id, affiliate_id, status, commission_amount, created_at, affiliates(ref_code, profiles:user_id(full_name))")
    .eq("status", "pending")
    .order("created_at", { ascending: false })
    .limit(50);
    ...
    <>
      ...
      <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="card-dark p-5">
          ...
          {convRows.length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-8">Không có conversion nào đang chờ duyệt.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
