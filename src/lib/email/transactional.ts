...
function getBaseUrl(): string {
  return process.env.NEXT_PUBLIC_APP_URL || "https://doanhnghiep1nguoi.online";
}

function getSiteDomain(): string {
  try { return new URL(getBaseUrl()).hostname; } catch { return "doanhnghiep1nguoi.online"; }
}

function getSiteName(): string {
  return process.env.EMAIL_FROM_NAME || "Doanh Nghiệp 1 Người";
}

// ─── Templates ───────────────────────────────────────────────────

function baseTemplate(content: string, recipientEmail?: string) {
...
<body>
  <div class="wrap">
    ...
    <div class="footer">
      ...
      <a href="${baseUrl}/unsubscribe?email=${encodeURIComponent(recipientEmail || '')}" style="color:#4b5563;">Huỷ đăng ký</a>
    </div>
  </div>
</body>
</html>`;
...
export async function sendWelcomeEmail(to: string, name: string) {
  return sesSendEmail(
    to,
    `Chào mừng ${escapeHtml(name)} đến với Doanh Nghiệp 1 Người! 🎉`,
    baseTemplate(`
      <h1>Chào mừng bạn, ${escapeHtml(name)}! 🚀</h1>
      <p>Tôi là <span class="highlight">Thiên Huệ</span> — và tôi rất vui khi bạn tham gia cộng đồng.</p>
      ...
      <p style="margin:8px 0 0; color:#6b7280; font-size:13px;">— Thiên Huệ</p>
    `),
  );
}
...
export async function sendPasswordResetEmail(
  ...
  return sesSendEmail(
    to,
    `🔑 Đặt lại mật khẩu — Doanh Nghiệp 1 Người`,
    baseTemplate(`
...
export async function sendAffiliateCommissionEmail(
  ...
  return sesSendEmail(
    to,
    `Bạn vừa nhận hoa hồng ${escapeHtml(formatted)} — Doanh Nghiệp 1 Người`,
    baseTemplate(`
...
export async function sendVerificationEmail(to: string, name: string, confirmUrl: string) {
  const html = baseTemplate(`
    ...
    <p>Cảm ơn bạn đã đăng ký tài khoản tại <span class="highlight">Doanh Nghiệp 1 Người</span>.</p>
  ...
  return sesSendEmail(
    to,
    "Xác thực tài khoản Doanh Nghiệp 1 Người",
    html,
  );
}
...
export async function sendLeadMagnetEmail(to: string, name?: string) {
  ...
  const html = baseTemplate(`
    <h2 style="color:#E85D04;margin:0 0 16px;">Đây là checklist của ${displayName}! 🎉</h2>
    <p style="margin:0 0 16px;color:#d1d5db;">Cảm ơn bạn đã đăng ký. Dưới đây là <strong>5 việc đang làm tay mà bạn có thể giao cho AI ngay hôm nay</strong>:</p>

    <div style="background:#1a1a1a;border-radius:12px;padding:20px;margin:0 0 20px;">
      <p style="margin:0 0 12px;color:#f5f5f5;font-weight:700;">✅ VIỆC 1-2: Tự động hoá email & tin nhắn</p>
      <p style="margin:0 0 16px;color:#9ca3af;font-size:14px;">Trả lời khách hàng, gửi báo giá, follow-up — không cần ngồi gõ thủ công mỗi ngày.</p>

      <p style="margin:0 0 12px;color:#f5f5f5;font-weight:700;">✅ VIỆC 3-4: Tổng hợp báo cáo tự động</p>
      <p style="margin:0 0 16px;color:#9ca3af;font-size:14px;">Kéo dữ liệu từ nhiều nguồn, tổng hợp và gửi báo cáo mỗi sáng.</p>

      <p style="margin:0 0 12px;color:#f5f5f5;font-weight:700;">✅ VIỆC 5: Lên lịch & nhắc việc thông minh</p>
      <p style="margin:0 0 0;color:#9ca3af;font-size:14px;">AI quản lý lịch, nhắc deadline, điều phối công việc team.</p>
    </div>

    <p style="margin:0 0 16px;color:#d1d5db;">Muốn áp dụng ngay? Bộ workflow n8n đóng gói sẵn của Thiên Huệ giúp bạn tự động hoá những việc trên chỉ trong 30 phút:</p>

    <div style="text-align:center;margin:24px 0;">
      <a href="${getBaseUrl()}/courses"
         style="background:#E85D04;color:#fff;padding:14px 32px;border-radius:10px;text-decoration:none;font-weight:700;display:inline-block;">
        Xem Workflow n8n — 149.000đ →
      </a>
    </div>
  `);
  ...
  return sesSendEmail(
    to,
    `[Doanh Nghiệp 1 Người] Checklist 5 việc giao cho AI của bạn`,
    html,
  );
}

export async function sendConfirmationEmail(to: string) {
  const confirmUrl = `${process.env.NEXT_PUBLIC_APP_URL || "https://doanhnghiep1nguoi.online"}/api/subscribe/confirm?email=${encodeURIComponent(to)}`;
  const html = baseTemplate(`
    <h1>Xác nhận đăng ký newsletter 📬</h1>
    <p>Cảm ơn bạn đã đăng ký nhận tin từ <span class="highlight">Doanh Nghiệp 1 Người</span>!</p>
    <p>Nhấn nút bên dưới để xác nhận email và bắt đầu nhận nội dung hữu ích mỗi tuần:</p>
    <div style="text-align:center;margin:28px 0;">
      <a href="${escapeHtml(confirmUrl)}"
         style="display:inline-block;padding:14px 32px;background:#E85D04;color:#fff;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;">
        Xác nhận đăng ký →
      </a>
    </div>
    <p style="font-size:13px;color:#6b7280;">Nếu nút không hoạt động, copy và dán link sau vào trình duyệt:<br/>
    <a href="${escapeHtml(confirmUrl)}" style="color:#E85D04;word-break:break-all;font-size:11px;">${escapeHtml(confirmUrl)}</a></p>
    <div class="divider"></div>
    <p style="margin:0;font-size:12px;color:#4b5563;">Nếu bạn không đăng ký, hãy bỏ qua email này.</p>
  `);

  return sesSendEmail(
    to,
    "Xác nhận đăng ký — Doanh Nghiệp 1 Người",
    html,
  );
}
