...
const DEFAULT_FROM_EMAIL = process.env.EMAIL_FROM || "support@doanhnghiep1nguoi.online";
const DEFAULT_FROM_NAME = process.env.EMAIL_FROM_NAME || "Doanh Nghiệp 1 Người";
const COMPANY_NAME = process.env.EMAIL_FROM_NAME || "Doanh Nghiệp 1 Người";
...
export async function sendSingleEmail(
  ...
  const result = await sendEmailWithParams({
    ...
    html: params.html,
    text: params.text,
    fromName: params.fromName || DEFAULT_FROM_NAME,
    fromEmail: params.fromEmail || DEFAULT_FROM_EMAIL,
  ...
  if (!result.success || !result.messageId) {
    throw new Error(result.error || "Resend did not return a message id");
  }
